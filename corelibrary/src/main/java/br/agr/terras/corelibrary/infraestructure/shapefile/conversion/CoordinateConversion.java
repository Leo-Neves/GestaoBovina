package br.agr.terras.corelibrary.infraestructure.shapefile.conversion;

/**
 * Created by leo on 01/09/17.
 */
...
public class CoordinateConversion {
    ...
    public double[] utm2LatLon(int zone, char hemisferio, double easting, double northing) {
        String UTM = String.format(Locale.US, "%02d", zone)+ " "+hemisferio+ " "+easting+ " "+northing;
    ...
    private class LatLon2UTM {
        public String convertLatLonToUTM(double latitude, double longitude) {
            ...
            UTM = longZone + " " + latZone + " " + ((int) _easting) + " "
                    + ((int) _northing);
            // UTM = longZone + " " + hemisferio + " " + decimalFormat.format(_easting) +
    ...
    private class UTM2LatLon {
        ...
        public double[] convertUTMToLatLong(String UTM) {
            ...
            String hemisphere = utm[1];
            double latitude = 0.0;
            double longitude = 0.0;

            if ((!hemisphere.equals("S")) && (!hemisphere.equals("N")) )
            {
                throw new IllegalArgumentException(
