...
public class ScaleBar extends android.support.v7.widget.AppCompatImageView {
    ...
    private void drawScaleBarPicture(Canvas canvas) {
        ...
        if (mMap==null){
            Log.e("ScaleBar", "map é null");
        }
        Projection projection = mMap==null?null:mMap.getProjection();
    ...
    private String scaleBarLengthText(float meters, boolean imperial, boolean nautical) {
        if (this.mIsImperial) {
            ...
            if (meters >= 1000) {
                return String.format(MyDateUtils.getLocaleBR(), "%.02f",(meters / 1000)) + "km";
            } /*else if (meters > 100) {
                return ((meters / 100.0) / 10.0) + "km";
            }*/ else {
                return String.format(MyDateUtils.getLocaleBR(), "%.02f",meters) + "m";
            }
        }
    }
}