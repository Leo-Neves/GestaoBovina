...
public class TabHostOld extends RelativeLayout implements View.OnClickListener {
    ...
    public TabHostOld(Context context) {
        this(context, null);
    }

    public TabHostOld(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public TabHostOld(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        // get attributes
        if(attrs != null) {
            TypedArray a = context.getTheme().obtainStyledAttributes(attrs,R.styleable.TabHostOld, 0, 0);

            try {
                // custom attributes
                hasIcons = a.getBoolean(R.styleable.TabHostOld_th_hasIcons, false);
                TypedValue typedValue = new TypedValue();
                context.getTheme().resolveAttribute(R.attr.colorPrimary, typedValue, true);
                int pColor = typedValue.data;
                primaryColor = a.getColor(R.styleable.TabHostOld_th_primaryColor, pColor);
                accentColor = a.getColor(R.styleable.TabHostOld_th_accentColor,Color.parseColor("#00b0ff"));
                iconColor = a.getColor(R.styleable.TabHostOld_th_iconColor,Color.WHITE);
                textColor = a.getColor(R.styleable.TabHostOld_th_textColor,Color.WHITE);
            } finally {
                a.recycle();
            }
        }
        ...
        this.isInEditMode();
        scrollable = false;
        isTablet = this.getResources().getBoolean(R.bool.md_is_tablet);
    ...
    public Tab newTab() {
        return new Tab(this.getContext(),hasIcons);
    }

    public void setSelectedNavigationItem(int position) {
        if(position < 0 || position > tabs.size()) {
            throw new RuntimeException("Invalid index "+position+" size is "+tabs.size());
        } else {
