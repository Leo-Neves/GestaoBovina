...
import br.agr.terras.materialdroid.childs.listview.WrapperViewList;
import br.agr.terras.materialdroid.utils.listview.AdapterWrapper;
import br.agr.terras.materialdroid.utils.listview.SectionIndexerAdapterWrapper;
...
public class ListView extends FrameLayout {
    ...
    public interface OnHeaderClickListener {
        void onHeaderClick(ListView l, View header,
    ...
    public interface OnStickyHeaderOffsetChangedListener {
         ...
         */
        void onStickyHeaderOffsetChanged(ListView l, View header, int offset);
    }
    ...
    public interface OnStickyHeaderChangedListener {
         ...
         */
        void onStickyHeaderChanged(ListView l, View header,
    ...
    public ListView(Context context, AttributeSet attrs, int defStyle) {
        ...
        // null out divider, dividers are handled by adapter so they look good com headers
        ...
        if (attrs != null) {
            TypedArray a = context.getTheme().obtainStyledAttributes(attrs,R.styleable.ListView, defStyle, 0);

            try {
                // -- View attributes --
                int padding = a.getDimensionPixelSize(R.styleable.ListView_android_padding, 0);
                mPaddingLeft = a.getDimensionPixelSize(R.styleable.ListView_android_paddingLeft, padding);
                mPaddingTop = a.getDimensionPixelSize(R.styleable.ListView_android_paddingTop, padding);
                mPaddingRight = a.getDimensionPixelSize(R.styleable.ListView_android_paddingRight, padding);
                mPaddingBottom = a.getDimensionPixelSize(R.styleable.ListView_android_paddingBottom, padding);

                setPadding(mPaddingLeft, mPaddingTop, mPaddingRight, mPaddingBottom);

                // Set clip to padding on the list and reset value to default on
                // wrapper
                mClippingToPadding = a.getBoolean(R.styleable.ListView_android_clipToPadding, true);
                super.setClipToPadding(true);
                mList.setClipToPadding(mClippingToPadding);

                // scrollbars
                final int scrollBars = a.getInt(R.styleable.ListView_android_scrollbars, 0x00000200);
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.GINGERBREAD) {
                    mList.setOverScrollMode(a.getInt(R.styleable.ListView_android_overScrollMode, 0));
                }

                // -- ListView attributes --
                mList.setFadingEdgeLength(a.getDimensionPixelSize(R.styleable.ListView_android_fadingEdgeLength,
                        mList.getVerticalFadingEdgeLength()));
                final int fadingEdge = a.getInt(R.styleable.ListView_android_requiresFadingEdge, 0);
                ...
                mList.setCacheColorHint(a
                        .getColor(R.styleable.ListView_android_cacheColorHint, mList.getCacheColorHint()));
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
                    mList.setChoiceMode(a.getInt(R.styleable.ListView_android_choiceMode,
                            mList.getChoiceMode()));
                }
                mList.setDrawSelectorOnTop(a.getBoolean(R.styleable.ListView_android_drawSelectorOnTop, false));
                mList.setFastScrollEnabled(a.getBoolean(R.styleable.ListView_android_fastScrollEnabled,
                        mList.isFastScrollEnabled()));
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
                    mList.setFastScrollAlwaysVisible(a.getBoolean(
                            R.styleable.ListView_android_fastScrollAlwaysVisible,
                            mList.isFastScrollAlwaysVisible()));
                }

                mList.setScrollBarStyle(a.getInt(R.styleable.ListView_android_scrollbarStyle, 0));

                if (a.hasValue(R.styleable.ListView_android_listSelector)) {
                    mList.setSelector(a.getDrawable(R.styleable.ListView_android_listSelector));
                }

                mList.setScrollingCacheEnabled(a.getBoolean(R.styleable.ListView_android_scrollingCache,
                        mList.isScrollingCacheEnabled()));

                if (a.hasValue(R.styleable.ListView_android_divider)) {
                    mDivider = a.getDrawable(R.styleable.ListView_android_divider);
                }

                mList.setStackFromBottom(a.getBoolean(R.styleable.ListView_android_stackFromBottom, false));

                mDividerHeight = a.getDimensionPixelSize(R.styleable.ListView_android_dividerHeight,
                        mDividerHeight);

                mList.setTranscriptMode(a.getInt(R.styleable.ListView_android_transcriptMode,
                        android.widget.ListView.TRANSCRIPT_MODE_DISABLED));

                // -- StickyListHeaders attributes --
                mAreHeadersSticky = a.getBoolean(R.styleable.ListView_hasStickyHeaders, true);
                mIsDrawingListUnderStickyHeader = a.getBoolean(
                        R.styleable.ListView_isDrawingListUnderStickyHeader,
                        true);
            } finally {
                a.recycle();
            }
    ...
    private void updateOrClearHeader(int firstVisiblePosition) {
        ...
        // but that does not work great together com getChildAt()
    ...
    private class AdapterWrapperHeaderClickHandler implements
        ...
        public void onHeaderClick(View header, int itemPosition, long headerId) {
            mOnHeaderClickListener.onHeaderClick(
                    ListView.this, header, itemPosition,
                    headerId, false);
        }
    ...
    /**
     * Use the method com extreme caution!! Changing any values on the
        ...
        if (l != null) {
            mList.setOnTouchListener(new OnTouchListener() {
                ...
                public boolean onTouch(View v, MotionEvent event) {
                    return l.onTouch(ListView.this, event);
                }
            });
        } else {
