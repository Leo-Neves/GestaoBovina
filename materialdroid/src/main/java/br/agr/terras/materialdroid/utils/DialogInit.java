...
import br.agr.terras.materialdroid.EditText;
...
import br.agr.terras.materialdroid.utils.progressbar.HorizontalProgressDrawable;
import br.agr.terras.materialdroid.utils.progressbar.IndeterminateHorizontalProgressDrawable;
import br.agr.terras.materialdroid.utils.progressbar.IndeterminateProgressDrawable;
...
public class DialogInit {
    ...
    @StyleRes
    public static int getTheme(@NonNull Dialog.Builder builder) {
    ...
    @LayoutRes
    public static int getInflateLayout(Dialog.Builder builder) {
    ...
    @UiThread
    public static void init(final Dialog dialog) {
        final Dialog.Builder builder = dialog.mBuilder;
...
//            }
        ...
        if (dialog.listView != null && (builder.items != null && builder.items.length > 0 || builder.adapter != null)) {
            ...
            // No custom adapter specified, setup the list com a DialogAdapter.
            // Which supports regular lists and single/multi choice dialogs.
            if (builder.adapter == null) {
                // Determine list type
                if (builder.listCallbackSingleChoice != null) {
                    dialog.listType = Dialog.ListType.SINGLE;
                } else if (builder.listCallbackMultiChoice != null) {
                    dialog.listType = Dialog.ListType.MULTI;
                    ...
                    dialog.listType = Dialog.ListType.REGULAR;
                }
                builder.adapter = new DefaultAdapter(dialog,
                        Dialog.ListType.getLayoutForType(dialog.listType));
    ...
    private static void setupProgressDialog(final Dialog dialog) {
        final Dialog.Builder builder = dialog.mBuilder;
    ...
    private static void setupInputDialog(final Dialog dialog) {
        final Dialog.Builder builder = dialog.mBuilder;
