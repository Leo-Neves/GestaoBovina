...
import br.agr.terras.materialdroid.childs.LineType;
...
public class TimelineView extends View {
    ...
    private void init(AttributeSet attrs) {
        ...
        mMarker = typedArray.getDrawable(R.styleable.Timeline_tl_marker);
        mMarkerColor = typedArray.getColor(R.styleable.Timeline_tl_markerColor, Color.GREEN);
        mStartLine = typedArray.getDrawable(R.styleable.Timeline_tl_line);
        mEndLine = typedArray.getDrawable(R.styleable.Timeline_tl_line);
        mMarkerSize = typedArray.getDimensionPixelSize(R.styleable.Timeline_tl_markerSize, 25);
        mLineSize = typedArray.getDimensionPixelSize(R.styleable.Timeline_tl_lineSize, 2);
        mLineOrientation = typedArray.getInt(R.styleable.Timeline_tl_lineOrientation, 1);
        mMarkerInCenter = typedArray.getBoolean(R.styleable.Timeline_tl_markerInCenter, true);
        typedArray.recycle();

        if(mMarker == null) {
            mMarker = mContext.getResources().getDrawable(R.drawable.timeline_marker);
        }
        mMarker.setColorFilter(mMarkerColor, PorterDuff.Mode.MULTIPLY);
