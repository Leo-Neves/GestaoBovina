...
import br.agr.terras.materialdroid.utils.RippleManager;
import br.agr.terras.materialdroid.utils.ThemeManager;
import br.agr.terras.materialdroid.utils.ThemeUtil;
import br.agr.terras.materialdroid.utils.TypefaceUtil;
import br.agr.terras.materialdroid.utils.ViewUtil;

/**
 * Created by leo on 18/05/16.
 */
public class SeekBar extends View implements ThemeManager.OnThemeChangedListener{
    ...
    public interface OnPositionChangeListener{
         ...
         */
        public void onPositionChanged(SeekBar view, boolean fromUser, float oldPos, float newPos, int oldValue, int newValue);
    }
    ...
    protected void applyStyle(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes){
        ...
        for(int i = 0, count = a.getIndexCount(); i < count; i++){
            ...
            if(attr == R.styleable.SeekBar_sb_discreteMode)
                mDiscreteMode = a.getBoolean(attr, false);
            else if(attr == R.styleable.SeekBar_sb_primaryColor)
                mPrimaryColor = a.getColor(attr, 0);
            else if(attr == R.styleable.SeekBar_sb_secondaryColor)
                mSecondaryColor = a.getColor(attr, 0);
            else if(attr == R.styleable.SeekBar_sb_trackSize)
                mTrackSize = a.getDimensionPixelSize(attr, 0);
            else if(attr == R.styleable.SeekBar_sb_trackCap) {
            ...
            else if(attr == R.styleable.SeekBar_sb_thumbBorderSize)
                mThumbBorderSize = a.getDimensionPixelSize(attr, 0);
            else if(attr == R.styleable.SeekBar_sb_thumbRadius)
                mThumbRadius = a.getDimensionPixelSize(attr, 0);
            else if(attr == R.styleable.SeekBar_sb_thumbFocusRadius)
                mThumbFocusRadius = a.getDimensionPixelSize(attr, 0);
            else if(attr == R.styleable.SeekBar_sb_travelAnimDuration) {
                mTravelAnimationDuration = a.getInteger(attr, 0);
                mTransformAnimationDuration = mTravelAnimationDuration;
            }
            else if(attr == R.styleable.SeekBar_sb_alwaysFillThumb) {
                mAlwaysFillThumb = a.getBoolean(R.styleable.SeekBar_sb_alwaysFillThumb, false);
            }
            else if(attr == R.styleable.SeekBar_sb_interpolator){
                int resId = a.getResourceId(R.styleable.SeekBar_sb_interpolator, 0);
                mInterpolator = AnimationUtils.loadInterpolator(context, resId);
            }
            else if(attr == R.styleable.SeekBar_android_gravity)
                mGravity = a.getInteger(attr, 0);
            else if(attr == R.styleable.SeekBar_sb_minValue) {
                minValue = a.getInteger(attr, 0);
                valueRangeDefined = true;
            }
            else if(attr == R.styleable.SeekBar_sb_maxValue) {
                maxValue = a.getInteger(attr, 0);
                valueRangeDefined = true;
            }
            else if(attr == R.styleable.SeekBar_sb_stepValue)
                mStepValue = a.getInteger(attr, 0);
            else if(attr == R.styleable.SeekBar_sb_value) {
                value = a.getInteger(attr, 0);
                valueDefined = true;
            }
            else if(attr == R.styleable.SeekBar_sb_fontFamily) {
                familyName = a.getString(attr);
                textStyleDefined = true;
            }
            else if(attr == R.styleable.SeekBar_sb_textStyle) {
                style = a.getInteger(attr, 0);
                textStyleDefined = true;
            }
            else if(attr == R.styleable.SeekBar_sb_seekBarTextColor)
                mTextColor = a.getColor(attr, 0);
            else if(attr == R.styleable.SeekBar_sb_seekBarTextSize)
                mTextSize = a.getDimensionPixelSize(attr, 0);
            else if(attr == R.styleable.SeekBar_android_enabled)
                setEnabled(a.getBoolean(attr, true));
            else if(attr == R.styleable.SeekBar_sb_baselineOffset)
    ...
    public void setDiscreteMode(boolean isDiscrete){
        this.mDiscreteMode = isDiscrete;
        invalidate();
    }

    public boolean isDiscreteMode(){
        return mDiscreteMode;
    }
    ...
    /**
     * Set the selected value of this SeekBar.
    ...
    static class SavedState extends BaseSavedState {
        ...
        /**
         * Constructor called from {@link SeekBar#onSaveInstanceState()}
         */
